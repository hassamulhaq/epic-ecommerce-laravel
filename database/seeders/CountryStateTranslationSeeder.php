...
class CountryStateTranslationSeeder extends Seeder
{
    public function run()
    {
        ...
        foreach (['ar', 'es', 'fa', 'pt_BR'] as $code) {
            ...
            $countryTranslations = json_decode(file_get_contents(__DIR__ . '/../../resources/data/country_state_translation/countries_' . $code . '.json'), true);

            data_fill($countryTranslations, '*.locale', $code);

            $stateTranslations = json_decode(file_get_contents(__DIR__ . '/../../resources/data/country_state_translation/states_' . $code . '.json'), true);
